...
docller.projectId = 0;
docller.folderId = 0;
docller.device = { MOBILE: 0, TABLET: 1, DESKTOP: 2 }


docller.utils = (function () {
    return {
        currentDevice:function() {
         
                if ($("#phoneSize").css("display") == "block") {
                    return docller.device.MOBILE;
                } else if ($("#tabletSize").css("display") == "block") {
                    return docller.device.TABLET;
                }
                return docller.device.DESKTOP;
        }
...
$(function () {
    ...
    var loadAndShowDialog = function (id, link, url) {
        ...
        $.get(url + separator + 'content=1')
            .done(function (content) {
                dialogs[id] = $('<div class="lmodal-popup">' + content + '</div>')
    ...
    $(".remoteModal").click(function (e) {
        e.preventDefault();
        var src = $(e.srcElement);
        var href = src.attr("href");
        var target = $(src.data("target"));

        target.find(".modal-body").empty().load(href);
        target.show();
    });
});